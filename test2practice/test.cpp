...
Matrix create_matrix(size_t rows, size_t cols) {
  int **mat = new int[rows];//creates matrix
  for (size_t i = 0; i < rows; i++) {
    mat[i] = new int[cols];//creates rows of matrix
    for (size_t j = 0; j < cols; j++) {
      mat[i][j] = 0;//assigns values to of 0 to matrix
    }
  }
...
Matrix& horzcat(Matrix& A, 
                ...
                size_t& rows_B, 
                size_t& cols_B) {
  if (rows_A != rows_B) {//throws exception if rowss dont match
    throw invalid_argument("rows dont match");
    return A;
  }
  int **c = create_matrix(rows_A, cols_A + cols_B);// creates temperorary matrix
  for (int i = 0; i < rows_A; i++) {
    for (int j = 0; j < cols_A; j++) {
      c[i][j] = A[i][j];//assigns A values to temporary
    }
    for (int j = cols_A; j < cols_A + cols_B; j++) {
      c[i][j] = B[i][j - cols_A];//assigns B values to temporary
    }
  }
  A = &c;//make A point to c(temperorary matrix)
